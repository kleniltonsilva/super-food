...
- (x) **Fase 7.1**: WA Sales Bot v2.1 — **Benjamim** + Áudio STT/TTS + Autonomia Total:
  - Nome: **Benjamim** (voz `rex`, masculino)
  - STT: transcrição áudios via Groq Whisper (grátis, 2000 req/dia)
  - TTS: xAI `/v1/tts` (NÃO `/v1/audio/speech`) — body: `text, voice_id, language, output_format`
  - Pronúncia: `_preparar_texto_tts()` converte "Derekh" → "Dérikh" somente em áudio
  - Áudio PTT nativo (bolinha verde) via Evolution `sendWhatsAppAudio` (NÃO `sendMedia`)
  - TTS autônomo: `_deve_enviar_audio()` (reciprocidade, explicações ≥3, conversa longa ≥10)
  - Retry automático backoff 5min → 30min → 2h
  - Cooling period anti-fadiga (N ações sem resposta → pausa)
  - Descoberta automática de regras → decisão pendente admin
